...
print('valori ed errori del coeff. angolare e del intercetta',m_hat, sigma_m, q_hat, sigma_q)
...
plt.savefig('massa_volume_ottone.pdf')
plt.show()
##ALLUMINIO
...
plt.ylabel('Massa [g]')
plt.grid(which='both', ls='dashed', color='gray')
plt.savefig('massa_volume_alluminio.pdf')
plt.show()
...
plt.ylabel('Massa [g]')
plt.grid(which='both', ls='dashed', color='gray')
plt.savefig('massa_volume_acciaio.pdf')
plt.show()

##calcolo massa raggio
...
print('valori ottimali ed errori di raggio e massa',norm_hat, sigma_norm, index_hat, sigma_index)
x = np.linspace(4., 10., 100)
plt.plot(x, power_law(x, norm_hat, index_hat),'g')
plt.xscale('log')
plt.yscale('log')
